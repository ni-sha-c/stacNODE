...
if model == "MLP_skip":
    MSE_MS_path = "../plot/Vector_field/"+str(dyn_sys)+"/"+str(model)+"_MSE_fullbatch/best_model_MLPskip_MSE.pth"
    JAC_MS_path = "../plot/Vector_field/"+str(dyn_sys)+"/"+str(model)+"_Jacobian_fullbatch/best_model_MLPskip_JAC.pth"
    mse_ms_model = ODE_MLP_skip(y_dim=dim, n_hidden=512, n_layers=5).to(device)
    best_ms_model = ODE_MLP_skip(y_dim=dim, n_hidden=256, n_layers=5).to(device)
else:
    ...
    mse_mlp_model = ODE_MLP(y_dim=dim, n_hidden=512, n_layers=5).to(device)
    best_mlp_model = ODE_MLP(y_dim=dim, n_hidden=512, n_layers=5).to(device)
...
fig, axs = subplots(1, 3, figsize=(36,10))
cmap = cm.plasma
traj = [true_long, learned_long, mse_long]
title = ['TRUE', 'JAC', 'MSE']


for y in range(3):
    orbit = traj[y]
    axs[y].plot(orbit[0, 0], orbit[0, 2], '+', markersize=32, color=cmap.colors[0], linewidth=40)
    axs[y].scatter(orbit[:, 0], orbit[:, 2], c=orbit[:, 2], s = 6, cmap='plasma', alpha=0.5)
    axs[y].set_xlabel("X")
    axs[y].set_ylabel("Z")
    axs[y].tick_params(labelsize=42)
    axs[y].xaxis.label.set_size(42)
    axs[y].yaxis.label.set_size(42)
    axs[y].set_title(title[y], fontsize=42)
tight_layout()
